...
const WriteRusEng = () => {
  ...
  function result() {
    let engWordLength = engWord.join('').split(',').sort().join('').trim().length
    if (engWord && translationWord && answer !== '') {
      translationWord.style.color = '#fff'
      let num = 0;
        engWord.join('').split(',').sort().forEach((el:string) => {
        ...
        if (engWord.join('').split(',').sort().join('').trim() === answer.toLowerCase().split(',').sort().join('').trim()) {
          num = engWordLength
        }
        if (num === engWordLength) {
          translationWord.style.color = '#56FF9A'
          translationWord.textContent = answer
        } else if (num === 0) {
          translationWord.style.color = '#FF6565'
          translationWord.textContent = engWord.join('')
        }
        if (resBtn) {
  ...
  function check() {
    if (answerCheck) {
      answerCheck.textContent = engWord.join(' ')
    }
  }
    ...
    <main>
      <section className='block_write'>
        <Link to={MAINPAGE_ROUTE} className='btn_back btn__blue'>
          ...
          <h1 className='word'>{rusWord}</h1>
          <div className='ds'>
          <h2 className='translation'></h2>
          </div>
          <h2 style={{color: "white"}} className='answerCheck'></h2>
